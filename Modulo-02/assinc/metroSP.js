...
/*
const getUniqueNamesSeasons = () => {

    const namesSeasons = [];

    const promise = getMetrosSP()
    const listFromSeasons = promise.then((dados) => {
        dados.filter((item) => {
            return namesSeasons.push(item._linha)
        })
    })
    // console.log(listFromSeasons);
    

    console.log(namesSeasons);
...
//     .then((data) => data.json())
//     .then((results) =>{
//         results.estacoes.estacao[40]._linha
//     })
